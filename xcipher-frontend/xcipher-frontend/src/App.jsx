...
function App() {
  ...
  // Global 3D & Reveal effects for the premium sections
  useEffect(() => {
    let obs;
  ...
  return (
    <>
      {/* Background and Cursor persist across all views */}
      <Background />
      <Cursor />
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'landing' && (
          ...
          >
            ...
            <main>
              <div id="platform">
                <Hero 
                  onGetStarted={() => { console.log('Deploy Clicked'); setView('login'); }} 
                  onReadPlan={() => { console.log('Specs Clicked'); setView('execution_plan'); }} 
                />
              </div>
              <SplitSection />
              <div id="technology">
              ...
              {/* Premium Components Integrated into Quantum Theme */}
        ...
        {view === 'integrations' && (
          <motion.div key="integrations" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            <IntegrationsView onBack={() => setView('landing')} />
          </motion.div>
        )}

        {view === 'changelog' && (
          <motion.div key="changelog" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            <ChangelogView onBack={() => setView('landing')} />
          </motion.div>
        )}

        {view === 'blog' && (
          <motion.div key="blog" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            <BlogView onBack={() => setView('landing')} />
          </motion.div>
        )}

        {view === 'status' && (
          <motion.div key="status" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            <StatusView onBack={() => setView('landing')} />
          </motion.div>
        )}

        {view === 'privacy' && (
          <motion.div key="privacy" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            <LegalView onBack={() => setView('landing')} title="Privacy Policy" />
          </motion.div>
        )}

        {view === 'terms' && (
          <motion.div key="terms" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            <LegalView onBack={() => setView('landing')} title="Terms of Service" />
          </motion.div>
        )}

        {view === 'contact' && (
          <motion.div key="contact" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
